...
class CpvClassifier(object):
    ...
    def load_model_from_hf(self):
        try:
            ...
            self.logger.error(f"-- -- Error loading models from local storage: {e}")
            return None, None
    ...
    def predict_description(self, description):
        ...
        description_vectorized = self.vectorizer.transform([processed_description])
        probs = self.model.predict_proba(description_vectorized)

        max_prob = max(probs[0].tolist())  # Convert probs[0] to a list
